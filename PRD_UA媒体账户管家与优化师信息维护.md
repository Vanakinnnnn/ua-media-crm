...
#### 1.4 Facebook账户子模块（已迁移至权限管理模块）
- **模块定位**：原Facebook账户子模块已整合到"账户权限管理"模块中
- **功能保留**：Facebook BM与Ad Account的关系管理功能完整保留
- **数据展示**：
  - 外层：显示BM信息、Ad Account信息、Owner/Client关系
  - 内层：显示有权限的优化师及其权限配置
- **API集成**：继续从Facebook API获取最新的BM-Account关系数据
- **刷新功能**：支持申请刷新，通过确认对话框二次确认
- **筛选搜索**：支持按BM名称、账户名称、角色等维度筛选
- **访问方式**：通过侧边栏"权限管理"菜单进入，选择Facebook平台查看

**迁移说明**：
- 将Facebook账户管理功能统一到权限管理模块，避免功能分散
- 提供更完整的Facebook权限关系视图（BM→Account→Optimizer）
- 保持原有的API数据源和刷新机制不变
